...
model.add(keras.layers.Dense(units=300, activation='relu')) #5 points
...
model.add(keras.layers.Dense(units=100, activation='relu')) #5 points
...
model.add(keras.layers.Dense(units=10, activation='softmax')) #5 points

# Explain why the softmax activation function was used for the output layer.
print("The softmax action function was use for the output layer because the softmax function outputs\
probabilities, which can be compared with other probabilities to determine the model's prediction") #5 points
...
print("The size of the first hidden layer is (None, 300). None means the shape of the output of the first\
hidden layer is 1D. The total number of parameters of the first hidden layer is 0, which refers to the number\
of weights") #8 points
model.summary()
...
print("An epoch is one training iteration over the training set. A model may train for multiple epochs,\
iterating over the training set over and over again. Usually, an epoch is done in batches, where a sample\
of the instances in the training set are used per batch") #6 points
...
model.fit(x=x_train, y=y_train, epochs=20) #6 points
...
for i in range(10):
    plt.subplot(5, 2, i + 1)
    plt.title('Predicted label: ' + str(np.argmax(x_test[i]))) #6 points
